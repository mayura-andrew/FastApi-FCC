...
def get_posts(db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user), 
   ...
   # posts = db.query(models.Post).filter(
    ...
    # resutls = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(
...
def get_post(id: int, db: Session = Depends(database.get_db),  current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(models.Vote, models.Vote.post_id == models.Post.id, isrouter=True).group_by(models.Post.id).filter(models.Post.id == id).first()
    if not post:
